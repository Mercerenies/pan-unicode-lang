...
export async function filter(term: SymbolLit, state: Evaluator): Promise<void> {
...
async function filterTestFunc(value: AST, func: AST, state: Evaluator): Promise<number> {
...
async function runFilter(depth: number, list: AST, func: AST, state: Evaluator): Promise<AST[]> {
...
export async function map(term: SymbolLit, state: Evaluator): Promise<void> {
  ...
  const result = await runMap(depth, args, func, async function(args: AST[], func: AST) {
    state.push(...args);
    await tryCall(func, state);
    return state.pop();
  });
...
async function runMap(depth: number, args: AST[], func: AST, baseCase: (args: AST[], func: AST) => Promise<AST>): Promise<AST> {
...
export async function each(term: SymbolLit, state: Evaluator): Promise<void> {
  let [argCount, depth] = term.getNumMod(1, 1);
  if (depth === MAX_NUM_MODIFIER) {
    depth = Infinity;
  }
  ...
  const args = everything.slice(0, -1);
  const func = everything[everything.length - 1];
  await runMap(depth, args, func, async function(args: AST[], func: AST) {
    state.push(...args);
    await tryCall(func, state);
...
export async function nestedQuery(term: SymbolLit, state: Evaluator): Promise<void> {
...
// ignored.
export async function select(term: SymbolLit, state: Evaluator): Promise<void> {
  const [list, index0] = state.pop(2);
  let index: AST[];
  if (index0 instanceof NumberLit) {
    index = [index0];
...
function rebuild(model: StringLit | ArrayLit, values: AST[]): AST {
  if (model instanceof StringLit) {
    return new StringLit(new Str(values.map(function(x: AST) {
...
export async function nth(state: Evaluator, value: AST | undefined, index: number | AST): Promise<AST | undefined> {
...
export async function gradeUp(term: SymbolLit, state: Evaluator): Promise<void> {
  let list0: AST;
...
// to the list.
export function ravel(term: SymbolLit, state: Evaluator): void {
  let depth = term.getNumMod(1);
  if (depth === MAX_NUM_MODIFIER) {
    depth = Infinity;
  }
...
export function doRavel(depth: number, list: AST[]): AST[] {
  if (depth <= 0) {
    return list;
  } else {
    let result: AST[] = [];
...
export async function outerProduct(term: SymbolLit, state: Evaluator): Promise<void> {
...
export async function doOuterProduct(state: Evaluator, func: AST, arglists: AST[][], n: number, prefix: AST[]): Promise<AST> {
...
export function* cartesianProduct<T>(lists: T[][]): Iterable<T[]> {
  yield* cartesianProductRec(lists, 0, []);
}


function* cartesianProductRec<T>(lists: T[][], n: number, prefix: T[]): Iterable<T[]> {
  if (n >= lists.length) {
    yield prefix;
  } else {
...
export async function member(term: SymbolLit, state: Evaluator): Promise<void> {
  const [list0, needle] = state.pop(2);
  const list = isList(list0);
  let func: (x: AST) => Promise<boolean>;
  if (term.getPrimeMod() > 0) {
    func = async function(x: AST) {
      state.push(x);
...
export function reshape(term: SymbolLit, state: Evaluator): void {
